...
let gBooks = storageService.loadFromStorage(STORAGE_KEY) || [
  {
    "id": "OXeMG8wNskc",
...
function onAddBookGoogle(book) {
  gBooks.push({
    ...
    publishedDate: book.volumeInfo.publishedDate || 'none', description: book.volumeInfo.description || 'none', pageCount: book.volumeInfo.pageCount || 'none',
    categories: book.volumeInfo.categories || [], thumbnail: book.volumeInfo.imageLinks.thumbnail || 'none', language: book.volumeInfo.language || 'none', listPrice: {
      amount: utilService.getRandomIntInclusive(0,200),
...
function getNextBookId(bookId,direction){
 const books =storageService.loadFromStorage(STORAGE_KEY) || gBooks
 const currId = books.findIndex(book => bookId === book.id)
 if (currId===books.length-1 && direction === 1) return books[0].id
 if (currId ===0 && direction === -1)  return books[books.length-1].id
 const nextId = currId + direction
 return books[nextId].id
}

function getBookById(bookId) {
  // let books = storageService.loadFromStorage(STORAGE_KEY)
  // if (!books || !books.length) books = gBooks
  let books = gBooks
  storageService.saveToStorage(STORAGE_KEY,gBooks)
